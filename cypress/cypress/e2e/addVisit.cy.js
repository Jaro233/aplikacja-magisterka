describe('Add Visit for a Pet', () => {
  it('allows a user to add a new visit for a pet', () => {
    ...
    cy.get('body > div:eq(0) > div:eq(0) > div:eq(0) > ui-view:eq(0) > visits:eq(0) > form:eq(0) > div:eq(0) > input:eq(0)').type('2024-04-04'); // Use the correct format for your date
    cy.get('body > div:eq(0) > div:eq(0) > div:eq(0) > ui-view:eq(0) > visits:eq(0) > form:eq(0) > div:eq(1) > textarea:eq(0)').type('Checkup and vaccinations');
