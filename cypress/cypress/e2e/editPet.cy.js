describe('Edit Pet Details', () => {
  it('allows a user to edit pet details', () => {
    ...
    cy.get('body > div:eq(0) > div:eq(0) > div:eq(0) > ui-view:eq(0) > pet-form:eq(0) > form:eq(0) > div:eq(1) > div:eq(0) > input:eq(0)').clear().type('New Pet Name');

    // Step 7: Update the pet's birth date if necessary
    cy.get('body > div:eq(0) > div:eq(0) > div:eq(0) > ui-view:eq(0) > pet-form:eq(0) > form:eq(0) > div:eq(2) > div:eq(0) > input:eq(0)').clear().type('2020-04-01');

    // Step 8: Update the pet's type if there's a dropdown for it
    cy.get('body > div:eq(0) > div:eq(0) > div:eq(0) > ui-view:eq(0) > pet-form:eq(0) > form:eq(0) > div:eq(3) > div:eq(0) > select:eq(0)').select('dog');
